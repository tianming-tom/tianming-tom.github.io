...
var APP_ID = 'jEFnHIdMQdOE1wFdz835wQG4-gzGzoHsz';
var APP_KEY = 'BcTKMT7uq3A9Vj0AEChs6dL6';
    ...
    methods: {
        fetchTodos: function fetchTodos() {
            ...
            if (this.currentUser) {
                ...
                query.find(this.currentUser.username).then(function (todos) {
                    var avAllTodos = todos[0]; // 因为理论上
                    // AllTodos
                    // 只有一个，所以我们取结果的第一项
                    var id = avAllTodos.id;
                    _this.todoList = JSON.parse(avAllTodos.attributes.content); // 为什么有个
                    // attributes？因为我从控制台看到的
                    _this.todoList.id = id; // 为什么给 todoList
                    // 这个数组设置
                    // id？因为数组也是对象啊
                }, function (error) {
                    console.error(error);
        ...
        updateTodos: function updateTodos() {
            ...
            var dataString = JSON.stringify(this.todoList); // JSON
            // 在序列化这个有
            // id
            // 的数组的时候，会得出怎样的结果？
        ...
        saveTodos: function saveTodos() {
            ...
            acl.setReadAccess(_leancloudStorage2.default.User.current(), true); // 只有这个 user
            // 能读
            acl.setWriteAccess(_leancloudStorage2.default.User.current(), true); // 只有这个 user
            // 能写

            avTodos.set('content', dataString);
            avTodos.setACL(acl); // 设置访问控制
            avTodos.save().then(function (todo) {
                _this2.todoList.id = todo.id; // 一定要记得把 id 挂到
                // this.todoList
                // 上，否则下次就不会调用
                // updateTodos 了
                console.log('保存成功');
        ...
        removeTodo: function removeTodo(todo) {
            var index = this.todoList.indexOf(todo); // Array.prototype.indexOf
            // 是 ES 5 新加的
            // API
        ...
        getCurrentUser: function getCurrentUser() {
            ...
            if (current) {
                ...
                // 上面这句话看不懂就得看 MDN 文档了
                // 我的《ES 6
                // 新特性列表》里面有链接：https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment

                return {
                    id: id,
                    username: username,
                    createdAt: createdAt // 看文档：https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Operators/Object_initializer#ECMAScript_6%E6%96%B0%E6%A0%87%E8%AE%B0
                };
            } else {
                return null;
            }
